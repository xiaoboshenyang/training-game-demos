...
export const POINTS=[7,8,10,12,15,18], THRESHOLDS=[2,2,4,4,5,5];
...
export class Game {
 ...
 reset(){this.pool=shuffle(NAMES.map((_,id)=>id),this.random).slice(0,9);this.level=1;this.highestLevel=1;this.score=0;this.correctRun=0;this.wrongRun=0;this.round=0;this.target=undefined;this.history=[];this.stats=LABELS.map((label,i)=>({level:i+1,label,score:0,clears:0,errors:0}));this.next(true);}
 next(changed=false){const ids=Array.from({length:this.level+3},(_,i)=>i+1);if(changed||this.level===6)this.reference=shuffle(ids,this.random);if(changed||this.level>=5)this.answers=shuffle(ids,this.random);this.digits=shuffle(ids,this.random);this.highlight=this.level<=2?Math.floor(ids.length/2):Math.floor(this.random()*ids.length);if(this.digits[this.highlight]===this.target){const alternatives=ids.map((_,i)=>i).filter(i=>i!==this.highlight);const other=alternatives[Math.floor(this.random()*alternatives.length)];[this.digits[this.highlight],this.digits[other]]=[this.digits[other],this.digits[this.highlight]];}this.target=this.digits[this.highlight];this.round++;}
