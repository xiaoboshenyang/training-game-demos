...
function createRound({level=1,seed=1,order=createOrder(level,seed),settings={},onCatch=()=>{},onFull=()=>{}}={}){
 ...
 const speed=Math.max(.2,Number(settings.speed)||1)*.9,sizeScale=Math.max(.4,Number(settings.size)||1),density=Math.max(.3,Number(settings.density)||1);
