...
OrderUI.render(host,{mode:'result',order:order.items,customer:(serial%6)+1,assets:FRUIT_ASSETS,result:{success,got,wrong:snap.stack.filter(f=>!f.correct).map(f=>({id:f.id,name:FRUIT_ASSETS[f.id].name})),score:award,levelMessage:level!==old?'下一单进入等级 '+level:''},onNext:()=>{if(api.isInteractive()&&!finished)showOrder()}});
