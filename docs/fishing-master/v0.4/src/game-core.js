...
const BASIC_FISH_IDS = Object.freeze(['big', 'medium', 'small']);
const SPECIAL_IDS = Object.freeze(['turtle', 'starfish', 'seahorse']);
const FISH_IDS = Object.freeze([...BASIC_FISH_IDS, ...SPECIAL_IDS]);
const SPAWN_IDS = Object.freeze(['big', 'medium', 'small', 'shark', ...SPECIAL_IDS]);
const MAX_LEVEL = 6;
...
export class FishingGameCore {
  ...
  createStats() {
    return {
      ...
      upgrades: 0,
      downgrades: 0,
      byType: { big: 0, medium: 0, small: 0, turtle: 0, starfish: 0, seahorse: 0 },
    };
  }
  ...
  setFishSpeedScale(value) {
    ...
    for (const entity of this.entities) {
      if (BASIC_FISH_IDS.includes(entity.type)) entity.speed *= ratio;
    }
  ...
  getLaneWeights(laneId) {
    const levelConfig = LEVEL_CONFIG[this.level];
    if (laneId === 'middle') return levelConfig.middleWeights ?? levelConfig.shallowMidWeights;
    if (laneId === 'bottom') return levelConfig.deepWeights;
    if (laneId === 'top') return levelConfig.shallowMidWeights;
    return null;
  }
  ...
  selectSpawnType(laneId, roll = this.random()) {
    ...
    if (!lane) return null;
    const weights = this.getLaneWeights(laneId);
  ...
  selectFishType(laneId, roll = this.random()) {
    ...
    if (!lane) return null;
    const weights = this.getLaneWeights(laneId);
    const totalFishWeight = BASIC_FISH_IDS.reduce((total, type) => total + weights[type], 0);
    const target = clamp(Number.isFinite(roll) ? roll : 0, 0, 1 - Number.EPSILON) * totalFishWeight;
    let cumulative = 0;
    for (const type of BASIC_FISH_IDS) {
      cumulative += weights[type];
      if (target < cumulative) return type;
    }
    return 'small';
  ...
  spawnForLane(laneId) {
    ...
    if (
      FISH_TYPES[type]?.special
      && (
        this.countConcurrentSpecials() >= GAME_CONFIG.specialPresence.maximumConcurrent
        || this.countConcurrentSpecials(type) >= GAME_CONFIG.specialPresence.maximumPerType
      )
    ) {
      type = this.selectFishType(laneId);
    }
  ...
  spawnEntity(type, laneId, overrides = {}) {
    ...
    if (typeConfig.minimumLevel && this.level < typeConfig.minimumLevel) return null;
    if (typeConfig.allowedLanes && !typeConfig.allowedLanes.includes(laneId)) return null;
    ...
    const entity = {
      ...
      speed: (overrides.speed ?? (this.worldWidth + visualWidth) / typeConfig.crossingSeconds)
        * (BASIC_FISH_IDS.includes(type) ? this.fishSpeedScale : 1),
      visualWidth,
  ...
  countConcurrentSpecials(type = null) {
    return this.entities.filter((entity) => (
      FISH_TYPES[entity.type]?.special && (type === null || entity.type === type)
    )).length;
  }
