...
const spriteContainerScale = { big: 627 / 560, medium: 627 / 560, small: 627 / 550, shark: 627 / 570 };
const tailFrameDurationMs = { big: 220, medium: 190, small: 160, shark: 270 };
...
function renderHook(snapshot, actorRender) {
  ...
  if (attachedFishType && visible) {
    dom.caughtFish.style.backgroundImage = `url("${images[attachedFishType]}")`;
    const visibleWidth = Math.max(84, FISH_TYPES[attachedFishType].visualWidth * .72);
...
function debugSpawnAtHook(type) {
  ...
  if (type === 'shark' && previousLevel === 1) core.level = 2;
  const laneId = type === 'shark' ? 'top' : 'middle';
...
if (new URLSearchParams(location.search).has('test')) {
  window.addEventListener('keydown', (event) => {
    ...
    if (/^[1-6]$/.test(event.key)) {
      core.setLevel(Number(event.key), 'manual');
      handleEvents(core.drainEvents());
      render();
    }
