(function startCloudJourney(root) {
  ...
  const passSound = new Audio("assets/pass.wav");
  passSound.preload = "auto";
  passSound.volume = 0.4;
  const backgroundMusic = new Audio("assets/background.mp3");
  backgroundMusic.preload = "auto";
  backgroundMusic.loop = true;
  backgroundMusic.volume = 0.2;
  coinSound.id = "coin-audio";
  passSound.id = "pass-audio";
  backgroundMusic.id = "background-audio";
  document.body.append(coinSound, passSound, backgroundMusic);
  ...
  function processEvents() {
    while (eventCursor < session.events.length) {
      ...
      if (event.type === "collision") {
        showFeedback("碰撞 -30", "hit");
        announce("碰到逆风云，扣 30 分");
      }
  ...
  elements.speedPercent.addEventListener("input", () => {
    applyPlaytestChanges(
      ...
      `飞行速度调整为 ${elements.speedPercent.value}%`
    );
  });
