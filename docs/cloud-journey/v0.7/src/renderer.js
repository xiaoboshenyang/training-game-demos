(function attachCloudJourneyRenderer(root, factory) {
  ...
  function createRenderer(canvas) {
    ...
    function drawCoin(obstacle, elapsed) {
      if (!obstacle.coin) return;
      const coin = obstacle.coin;
      const age = coin.collected ? elapsed - coin.collectedAt : 0;
      if (coin.collected && (!Number.isFinite(age) || age >= CONFIG.coinPickupSeconds)) return;
      const progress = coin.collected ? Math.max(0, age / CONFIG.coinPickupSeconds) : 0;
      const size = CONFIG.coinDisplayRadius * 2;
      // Turn the original sprite only; keep a narrow visible profile at the edge.
      // Pickup uses its own fixed circle in game-core, independent of this drawing.
      const angle = elapsed * Math.PI * 2 / CONFIG.coinTurnSeconds + obstacle.index * 0.7;
      const faceWidth = coin.collected ? 1 : Math.abs(Math.cos(angle));
      const pop = coin.collected ? 1 + Math.sin(progress * Math.PI) * 0.18 : 1;
      context.save();
      context.translate(coin.collected ? coin.collectedX : core.coinPositionX(obstacle), coin.y);
      context.scale(pop, pop);
      context.globalAlpha = coin.collected ? 1 - progress : 1;
      const face = Math.max(size * 0.1, size * faceWidth);
      context.shadowBlur = 0;
      if (assets.coin) context.drawImage(assets.coin, -face / 2, -size / 2, face, size);
      else {
        context.fillStyle = "#e7a92b";
        context.beginPath();
        context.ellipse(0, 0, face / 2, size / 2, 0, 0, Math.PI * 2);
        context.fill();
      }
      context.restore();
    }
