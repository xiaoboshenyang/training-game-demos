(function () {
  ...
  const state = {
    ...
    commitInFlight: false, finishAfterCommit: false
  };
    ...
    for (let row = 0; row < snap.rows; row += 1) {
      for (let col = 0; col < snap.cols; col += 1) {
        ...
        if (filled.has(cellKey)) {
          ...
          node.setAttribute("aria-label", `第${row + 1}行第${col + 1}列，已填平，可通行`);
        }
  ...
  async function commitGesture() {
    ...
    renderBoard();
    // Render the repaired road immediately; pulse only the pit of this commit.
    const pit = route[route.length - 1];
    const filledCell = refs.board.querySelector(`[data-row="${pit[0]}"][data-col="${pit[1]}"]`);
    const pulse = document.createElement("span");
    pulse.className = "fill-pulse";
    pulse.setAttribute("aria-hidden", "true");
    filledCell?.append(pulse);
    await wait(500);
    pulse.remove();
    if (state.ended || generation !== state.generation) {
      state.commitInFlight = false;
    ...
    state.commitInFlight = false;
    state.stepIdleAt = performance.now();
    if (result.complete) await completeQuestion(state.finishAfterCommit);
    else if (state.finishAfterCommit) endRound();
    else {
      state.inputLocked = false;
  ...
  async function completeQuestion(finishAfter = false) {
    ...
    await wait(720);
    if (state.ended) return;
    if (finishAfter) { endRound(); return; }
  ...
  function startTimer() {
    ...
    state.timerId = setInterval(() => {
      ...
      if (!state.paused && !state.inputLockedHint && state.started && !state.ended) {
        ...
        if (state.timeLeft <= 0) expireRound();
  ...
  function resetRound(preserveTestMode = false) {
    clearInterval(state.timerId);
    if (!preserveTestMode) {
      state.mode = "auto";
      state.level = 1;
  ...
  function endRound() {
    ...
    state.commitInFlight = false; state.finishAfterCommit = false;
  ...
  $("#startButton").addEventListener("click", () => resetRound(false));
  $("#playAgainButton").addEventListener("click", () => resetRound(false));
  $("#restartButton").addEventListener("click", () => resetRound(false));
  ...
  if (query.get("autostart") === "1") {
    ...
    if (query.get("restart") === "1") setTimeout(() => resetRound(false), 200);
    if (query.get("timeout_commit") === "1") setTimeout(() => {
      window.__B_DEMO__.completeCurrentStep();
      setTimeout(expireRound, 80);
    }, 160);
  }
})();
