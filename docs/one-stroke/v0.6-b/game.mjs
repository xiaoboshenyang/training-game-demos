...
import {Puzzle,solve,moves,settle,LABELS,layout,BENCHMARK_SCORE} from './engine.mjs';
...
for(let l=1;l<=6;l++)pool['L'+l]=pool['L'+l].filter(q=>
  (l<2||q.edges.some(e=>e[2]===2))&&(l<5?q.edges.every(e=>e[3]===0):q.edges.filter(e=>e[3]===1).length===(l===5?1:2))&&(l<6||q.edges.filter(e=>e[3]===1).length>=2));
...
function snapshot(){
  return {totalScore:total,benchmarkScore:BENCHMARK_SCORE,levels:stats.map(s=>({...s})),highestLevel,history:history.map(h=>({...h}))};
}
function finishSession(){
  ...
  summary.innerHTML='<p>总分 <strong>'+total+'</strong> 分</p><p class="benchmark-score">基准分：'+BENCHMARK_SCORE+' 分</p><table aria-label="各等级成功失败统计"><thead><tr><th>难度</th><th>成功</th><th>失败</th><th>得分</th></tr></thead><tbody>'+stats.map(s=>'<tr><td>L'+s.level+' '+s.label+'</td><td>'+s.successes+'</td><td>'+s.failures+'</td><td>'+s.score+'</td></tr>').join('')+'</tbody></table>';
...
function newRound(){
  ...
  $('#instruction').textContent=level===1?'把所有灰线都走一遍':level<5?'双线走两遍，每次亮一条':'双线走两遍，箭头顺着走';
