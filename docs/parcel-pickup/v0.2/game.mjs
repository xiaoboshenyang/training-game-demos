...
function render(){
 ...
 if(memory){board.innerHTML='<div class="person-wrap"><img class="person" src="assets/person.png" alt="微笑着托你取快递的邻居"></div><section class="memory-card"><h1>帮我取个快递吧！</h1><p class="label">取件码是</p><div id="memory-code" class="memory-code" aria-label="取件码 '+question.code.split('').join(' ')+'">'+digits(question.code,question.code.length)+'</div><p class="hint">记住这 '+question.code.length+' 位数字</p><button id="remember" class="primary remember">记好了，去取件 →</button></section>';board.querySelector('#remember').onclick=()=>{if(!interactive()||phase!=='memory')return;phase='input';render();};}
 else {board.innerHTML=cabinet(success)+'<section class="keypad-panel"><h1>请输入取件码</h1><div id="entered-code" class="entered-code" role="status" aria-label="已输入 '+input.split('').join(' ')+'">'+digits(input,question.code.length)+'</div><div class="keypad">'+[1,2,3,4,5,6,7,8,9,'删除',0,'确认'].map(k=>'<button class="key '+(k==='确认'?'confirm':'')+'" data-key="'+k+'" '+(done?'disabled':'')+'>'+k+'</button>').join('')+'</div><p class="input-hint" id="input-hint" role="status">'+(done?'本题已结束':'输入后，点击确认')+'</p></section>';board.querySelectorAll('[data-key]').forEach(b=>b.onclick=()=>press(b.dataset.key));}
 musicSync();renderDev();
}
...
function press(key){if(!interactive()||phase!=='input')return;
 if(key==='确认'){const result=judge(session,question,input);if(!result){board.querySelector('#input-hint').textContent='请输满取件码';return;}question.result=result;phase='feedback';render();api.feedback({correct:result.correct,score:session.score,level:session.level,levelUp:result.changed>0,onComplete:next});return;}
 ...
 const box=board.querySelector('#entered-code');box.innerHTML=digits(input,question.code.length);box.setAttribute('aria-label','已输入 '+input.split('').join(' '));board.querySelector('#input-hint').textContent='输入后，点击确认';
}
