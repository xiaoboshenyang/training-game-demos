...
export const ROCK_SKINS=['obstacle_rock.png','obstacle_flat.png','obstacle_rounded.png','obstacle_chipped.png'];
export async function loadAssets(){
  const names=['background.png','player_stone.png','finger.png',...ROCK_SKINS];
...
export class Renderer{
  constructor(canvas,assets){this.canvas=canvas;this.ctx=canvas.getContext('2d');this.assets=assets;this.rockSkins=new Map();}
  ...
  draw(state){
    ...
    const activeRocks=new Set();
    for(const row of state.rows)for(const rock of row.obstacles){
      const key=`${row.id}:${rock.side}`;activeRocks.add(key);
      if(!this.rockSkins.has(key))this.rockSkins.set(key,ROCK_SKINS[crypto.getRandomValues(new Uint32Array(1))[0]%ROCK_SKINS.length]);
      const name=this.rockSkins.get(key);
      ctx.save();if(row.settled)ctx.globalAlpha=.7;this.sprite(name,rock.x,row.y,this.assets.metrics[name].display_width??104,[652,636]);ctx.restore();
    }
    for(const key of this.rockSkins.keys())if(!activeRocks.has(key))this.rockSkins.delete(key);
