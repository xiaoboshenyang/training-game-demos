(function (root, factory) {
  ...
  class Engine {
    constructor({seed=Date.now(),level=1,locked=false,durationMs=120000,tutorial=false}={}) {
    ...
    _settle(expired,incomplete) {
      ...
      const points=this.tutorial?0:correct*5+(perfect && !s.assisted?[10,15,20,25,30,35][roundLevel-1]:0);
    ...
    tick(deltaMs) {
      ...
      if (s.phase==='feedback') {
        ...
        if (this._feedbackMs<=0) this._newRound();
      }
      return true;
    }
    pause() {
