...
const POINTS=[10,15,20,25,30,40], LENGTHS=[3,3,4,5,6,7];
class Game{
 ...
 newQuestion(){
  ...
  this.assisted=false;this.settled=false;this.index=0;this.nextLevel=this.level;this.prepare();
 }
 prepare(){this.phase='prepare';this.wait=1000;this.active=-1;this.errorKey=-1;this.litFor=0;this.index=0;}
 ...
 input(key){
  ...
  if(this.index===this.question.keys.length){this.settle(true);this.phase='keyFinish';this.afterKey='success';this.wait=500;this.litFor=500;}
  return true;
 }
 help(reason='manual'){
  ...
  if(reason==='wrong'){this.afterWrong=this.phase;this.phase='keyFinish';this.afterKey='wrong';this.wait=500;this.litFor=500;}
 ...
 advance(ms){
  ...
  while(ms>0&&!this.paused&&!['intro','end','ready'].includes(this.phase)){
   ...
   switch(this.phase){
    ...
    case 'prepare':this.play('demo');break;
    case 'keyFinish':this.phase=this.afterKey;this.wait=1000;this.active=-1;this.errorKey=-1;if(this.phase==='success')this.emit({type:'success'});break;
    case 'retry':this.prepare();break;
    case 'wrong':this.errorKey=-1;if(this.afterWrong==='retry')this.prepare();else this.newQuestion();break;
